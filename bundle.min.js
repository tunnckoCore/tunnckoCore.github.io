...
function poisonClick(e){button.removeEventListener('click',poisonClick,false)
...
if(e!==1){document.execCommand('copy')}
var ready=document.createElement('div')
ready.className='ui tag label green'
ready.textContent='Ready'
parent.replaceChild(ready,button)}
function poisoner(input){var chars='ABEKMHOΠPCTXaeĸopcyxIiJjSs'+'АВЕКМНОПРСТХаекорсухІіЈјЅѕ'
var middle=chars.length/2
var poison=''
for(var i=0;i<input.length;i++){var index=chars.indexOf(input.charAt(i))
if(index===-1){poison+=input.charAt(i)}else if(index>=middle){poison+=chars.charAt(index-middle)}else{poison+=chars.charAt(index+middle)}}
return poison}